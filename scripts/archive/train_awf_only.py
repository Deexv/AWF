"""Continue training AWF LLM with more epochs."""
...
for ep in range(10):
    ...
    dt = time.time() - t0
    print(f"  [awf] ep{ep+1:02d} train_loss={ep_loss/n_seen:.4f} val_loss={val_loss:.4f} val_acc={val_acc*100:.2f}% ppl={ppl:.1f} ({dt:.0f}s)")
    if val_acc > 0.9:
        print("  Reached 90% val acc — stopping early")
        break
...
print(f"\n=== FINAL ===")
print(f"Dense: {dense_params:,} params, {dense_params*4/1024:.1f}KB, val_loss={dense_val_loss:.4f}, val_acc={dense_val_acc*100:.2f}%")
